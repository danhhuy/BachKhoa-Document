...
inline ULL binary_to_ULL( const vector<bool>& binary )
{
...
{
	assert( s.size() == 8 ); // A string of 8 characters = 64 bits = ULL
...
inline string binary_to_HEX( const string& s )
{
	assert( s.size() % 4 == 0 );
...
ULL toggle_endianness( const ULL& ull )
{
...
const ULL RC[24] = { 0x0000000000000001,
                     0x0000000000008082,
                     0x800000000000808A,
                     0x8000000080008000,
                     0x000000000000808B,
...
void Round( vector<vector<ULL> >& S, const int& RC_index )
{
	//  Theta Step
	vector<ULL> C( 5 );
	for ( int i = 0; i < 5; i++ )
	...
	//Rho and pi steps
	vector<vector<ULL> > B( 5, vector<ULL> ( 5, 0 ) );
	for ( int i = 0; i < 5; i++ )
	{
		for ( int j = 0; j < 5; j++ )
		{
...
string SHA3_512( string message )
{
	...
	ULL message_block_count = ( message.size() * 8 ) / 576;
	vector<vector<ULL> > S( 5, vector<ULL> ( 5, 0 ) );
...
int main()
{
	...
	string digest = SHA3_512( message );
