...
def matrix2HexString(matrix):
    string = ''
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            temp = str(hex(matrix[i][j])[2:])
            if len(temp) == 1:
...
def hexString2Matrix(text, tag):
    encpImg = np.zeros((4,4), np.uint8)
    for i in range(4):
        for j in range(4):
            index = i*8 + j*2
            if tag == 0:
                encpImg[i][j] = np.uint8(int(text[0][index:index+2], 16))
            if tag == 1:
                encpImg[i][j] = np.uint8(int(text[index:index+2], 16))
            #print(text[0][index:index+2])#, encpImg[i][j])
    return encpImg

def gmul(a, b):
    if b == 1:
        return a
    tmp = (a << 1) & 0xffffffffffffffffffffffffffffffff
    if b == 2:
        return tmp if a < 2**128 else tmp ^ 0x87
    if b == 3:
        return gmul(tmp, 2)

def MSB(origTag, Tlen):
    Tag = int(origTag, 16) >> (128-Tlen)
    return hex(Tag)[2:]