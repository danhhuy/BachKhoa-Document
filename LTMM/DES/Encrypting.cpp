...
string DES(){ 
	...
	for(int i=0; i<16; i++) { 
        ...
        cout << "Round " << i << ":" << "left:" << std::hex << convertBinaryToDecimal(left) 
		<< " right:" << convertBinaryToDecimal(right) << " key:" << convertBinaryToDecimal(round_keys[i]) << endl;; 
	} 
	// 4. The halves of the plain text are applied
	string combined_text = left + right;   
	string ciphertext =""; 
	// The inverse of the initial permuttaion is applied
	cout << "Inverse Permutation: " << combined_text << endl;
	for(int i = 0; i < 64; i++){ 
